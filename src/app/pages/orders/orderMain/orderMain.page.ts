...
export class OrderMainPage implements OnInit {
  ...
  constructor(public plt: Platform, public notificationService: NotificationsService,private orderService: OrderService,
    public toastController: ToastController,public CurrencyRateService:CurrencyRateService) {
  ...
  doRefresh(event) {
    this.orderService.getDataFromDatabase().then(() => {
      this.getOrders();
    }).catch((error) => {
      this.orderList = [];
      if (error.status == 404) {
